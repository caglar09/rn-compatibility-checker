...
// GitHub API URL
const GITHUB_API_URL = 'https://raw.githubusercontent.com/react-native-community/directory/main/react-native-libraries.json';
...
async function getLocalCache() {
  const cacheDir = path.resolve(process.env.HOME || process.env.USERPROFILE, '.rn-compatibility-checker');
...
async function loadLibrariesData() {
  ...
  try {
    // Check cache status
    const { cachedData, needsUpdate, cachePath } = await getLocalCache();
    ...
    if (needsUpdate) {
      ...
      try {
        ...
        spinner.succeed(`Loaded ${librariesData.length} React Native libraries from GitHub.`);
        return librariesData;
      } catch (error) {
        // If data can't be fetched from GitHub and there's data in cache, use cache
        if (cachedData.length > 0) {
          spinner.warn(`Failed to fetch data from GitHub: ${error.message}. Using cache.`);
          return cachedData;
        }
      ...
      spinner.succeed(`Loaded ${cachedData.length} React Native libraries from cache.`);
      return cachedData;
    }
  } catch (error) {
    spinner.fail(`${chalk.red('Error:')} An error occurred while loading data source: ${error.message}`);
    
    // Try using backup JSON file in package
    try {
      const backupPath = path.resolve(__dirname, '../../react-native-libraries.json');
...
function findLibraryByPackageName(librariesData, packageName) {
  ...
  if (!library) {
    library = librariesData.find(lib => {
      ...
      return repoName === packageName.toLowerCase() || 
 ...
 */
async function checkPlatformCompatibility(dependencies, platforms, customLibrariesData) {
  ...
  try {
    ...
    spinner.fail(`${chalk.red('Error:')} An error occurred during compatibility check: ${error.message}`);
    process.exit(1);
  }
}
      ...
      if (result.found) {
        row.push(
          result.newArchitecture !== undefined ? 
            (result.newArchitecture ? 'true' : 'false') : 
            'unknown',
...
function formatResults(results, platforms, detailed = false, format = 'table') {
    ...
    results.forEach(result => {
      ...
      if (detailed) {
        if (result.found) {
          row.push(
            result.newArchitecture !== undefined ? 
              (result.newArchitecture ? chalk.green('✓') : chalk.red('✗')) : 
              chalk.yellow('?'),
  ...
  const platformStats = {};
  platforms.forEach(platform => {
    platformStats[platform] = results.filter(r => r.found && r.platforms[platform]).length;
  });
  ...
  platforms.forEach(platform => {
    ...
    console.log(`${platform}: ${chalk.green(compatibleCount)}/${foundCount} (${percentage}%)`);
  });
  
  if (detailed) {
    const newArchCount = results.filter(r => r.found && r.newArchitecture).length;
    const unmaintainedCount = results.filter(r => r.found && r.unmaintained).length;
    
    console.log('\nAdditional information:');
    console.log(`New Architecture support: ${chalk.green(newArchCount)}/${foundCount} (${Math.round((newArchCount / foundCount) * 100) || 0}%)`);
    console.log(`Unmaintained packages: ${chalk.red(unmaintainedCount)}/${foundCount} (${Math.round((unmaintainedCount / foundCount) * 100) || 0}%)`);
  }
}
